...
def create_keyboard(mapping = ["1234567890-","QWERTYUIOP", "ASDFGHJKL;'", "ZXCVBNM,.?"], starting_coordinate=(0, 0), size=50, spacing = 5):
    keys = {}

    # Number Row
    for index, key in enumerate(list(mapping[0])):
        x, y =(starting_coordinate[0] + -40 + (size +spacing) * index, starting_coordinate[1] + (size + spacing) * -1)
    ...
    # Third Row    
    for index, key in enumerate(list(mapping[3])):
...
def show_keyboard (img, keys, mapping = ["1234567890-","QWERTYUIOP", "ASDFGHJKL;'", "ZXCVBNM,.?"], starting_coordinate=(0, 0), size=50, spacing = 5):
